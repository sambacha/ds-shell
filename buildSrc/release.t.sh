...
if [[ "$(git cat-file -t "$current")" != "tag" ]]; then
  fail "[alert] Current tag is not annotated (required for mkdist)."
fi

if [[ "$(git tag --points-at master)" != "$current" ]]; then
  fail "[alert] You are not on master"
fi

version=${current#v}
if ! grep "Version:" dsshell | grep -qFw "$version"; then
...
if [[ $(git log -1 --pretty=%B) != "feat(release) "* ]]; then
