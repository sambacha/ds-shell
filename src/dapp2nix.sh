...
Commands
  init                      Create a .dsforge.json
  migrate                   Populate .dsforge.json from submodules
...
$PWD $con"
  ...
  for path in $(cut -d " " -f1 <<<"$repos"); do
    ...
    if [[ $path != "$root" ]]; then
      printf %s "$sep"; sep=","
      if [[ -f "$path/.dsforge.json" ]]; then
        jq .contracts "$path/.dsforge.json" | sed '1d;$d'
      else
        spec "$path"
      fi
    fi
...
tmpclone() {
  ...
  (cd "$target" || exit 3
    if [[ -f "./dsforge.json" ]]; then
      local repo; repo=$(jq -c .repo <<<"$(spec "$target" | sed '1s/^.*://')")
      jq ".this.repo = $repo" ./dsforge.json
    else
...
init() {
  cat \
    ...
    "${DSFORGE_EXPR:-${BASH_SOURCE[0]%/*}/dsforge.nix}" \
    > "$1/dsforge.nix"
  printf %s "
{
  \"contracts\": {},
...
case "${1:-help}" in
  init)
    [[ ! -e "$outPath" ]] || {
      echo >&2 ".dsforge.json already exists, will not init"
      exit 1
    }
  ...
  migrate)
    [[ ! -e "$outPath" ]] || {
      echo >&2 ".dsforge.json already exists, will not migrate"
      exit 1
    }
