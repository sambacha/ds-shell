...
print_help() {
    echo "Installs latest (or specific) version of $APP. Installation directory defaults to /usr/local/bin."
    echo -e
    echo "Usage:"
...
if [[ "$version" == "" ]]; then
    version="$(curl -sL https://raw.githubusercontent.com/$GITHUB_ORG/$REPO/master/resources/$RELEASE_CHANNEL)"
fi
...
case "$platform-$static_binary" in
    linux-true) filename="$APP-$version-$platform-$arch-static."$ext
                ;;
    *-true)     >&2 echo "Static binaries are only available in Linux platform! Using the non-static one..."
                filename="$APP-$version-$platform-$arch."$ext
                ;;
    *)          filename="$APP-$version-$platform-$arch."$ext
                ;;
esac
...
download_url="https://github.com/$GITHUB_ORG/$REPO/releases/download/v$version/$filename"
